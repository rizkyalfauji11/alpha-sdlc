...
**Read the project profile first** (`docs/basics/` from `do-project-setup`) — architecture, code structure, api-reference, database, conventions, `06-domain-model.md` (entity-integrity classes), `08-data-cache.md` (sync/staleness classes), `13-auth.md` (401/token classes), `18-design-tokens.md` (visual-drift classes — a reported "wrong padding / wrong text size" is almost always the whole class, so audit against the standard and the doc's *Contradictions found*, not just the reported screen), and especially **`16-feature-map.md`** — as your grounding reference before scanning code from scratch. You need it to know *where the issue class could live* and *which features it touches*. If a section looks stale (repo moved past its commit stamp), note it and suggest a refresh.
...
1. **Name the class.** Abstract the symptom to its pattern — e.g. "localized `{en,id}` object rendered raw", "FE request method/path ≠ backend route (→ 405)", "control type ≠ design", "unhandled null/empty from the API", "consumer's list stale — shared entity not invalidated (freshness)", "dangling reference after owner delete/archive (undecided on-delete edge)", "wizard step loses state / half-written commit", **"a section case that was never built"** (the empty state never shows, the footer stays visible offline, a role variant is missing).
2. **Search the whole project for that class.** Use the profile to know where to look (all clients for a render bug, all FE↔BE call sites for a contract bug), then scan the real code — list **every affected site**, not just the reported one. Say what you searched and any area you couldn't cover. **For a UI-behavior issue, the sweep runs over the `section-slicing/<screen>.md` docs**, not just the reported screen: the same case is usually missing on every sibling screen (if one screen's list has no empty state, check them all). Two outcomes to separate, because the fix differs: the case **is specced but wasn't built** (a build/coverage failure — the fix is to build it, and `do-testing` gains its case test), or the case **was never specced** (a grooming gap — the case is **added to the section-slicing doc** with its crop, register-on-create, before it can be fixed). Report which of the two the reported instance is.
...
4. **Prefer a systemic fix over N patches.** A root fix that eliminates the class (a shared helper, a generated typed client, a lint rule, a contract, the sync convention — canonical keys/invalidation per `08-data-cache.md` — a decided domain-model edge + DB constraint, a registered UX convention) beats patching each site — climb the ladder and name the rung. Per-site patching is the last resort; if you must, list every site so none is missed.
...
> Present every gate below in the shared **step-summary format** (`principles.md`): header (development · phase · step · status) · **What** (plain + engineer phrase) · **Why** (leads whenever a question is asked) · **Who** · **When** · **Where** · **How** (ends with what I need from you) · engineer detail (audit table, root cause) last.
