...
5. **Visual parity (UI stages with a design ref)** — render + screenshot the screen, compare to the design (AI checklist + pixel-diff), fix mismatches, re-render until parity within platform-best-practice tolerance. Skip for non-UI stages or when no design ref exists; if the app can't be rendered in-session, note it and ask the user to eyeball.
6. **Present + ⏸ STOP** — give the user a structured review packet, not just a raw diff:
   ...
   - **Visual parity (UI stages)** — the screenshot(s) next to the design, the AI checklist + pixel-diff result, and any accepted platform deviations. "n/a" for non-UI, or note if parity couldn't be rendered in-session.
