...
- **Comments are for the whole project, not the task.** Write comments that explain the code's intent/behavior for *any* future reader of the codebase — they must still make sense a year from now with the ticket long closed. **Never** reference the task, ticket key, stage, plan, or "added for X / as part of Y". No changelog-style or provenance comments in the source — that history belongs in git/PR/Jira. A comment earns its place by explaining *why the code is the way it is* in project terms, not *where it came from*.
- **Minimize comments — the best comment is usually none.** Prefer clear names and structure over prose. Comment *only* what the code can't say itself: a non-obvious *why*, a gotcha, a constraint, a money/security edge case. **No** comments restating what the code plainly does, no section-divider banners, no narrating every line. Keep each one short and plain — one line where possible. Match the surrounding file's comment density; if in doubt, leave it out.
