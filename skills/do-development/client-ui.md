...
> **Every prohibition still lives in `SKILL.md` and `../../principles.md`, and every one of them
> binds
> here unchanged.** Nothing in this file relaxes or replaces one: don't trust code-from-image ·
> never
> read a value off the mockup, zero raw literals · never substitute a component that merely looks
> close
> · a wrong token is a defect inside pixel tolerance · parity is not passed while any section is
> uncompared · never skip or auto-continue past a failed comparison, and never claim parity you
> didn't
> verify.
...
- **Every iteration lands in `docs/development/<feature-name>/design/compared-ui/`** — the actual
  ...
  never-commit rule are in `SKILL.md`.
