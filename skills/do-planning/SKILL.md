...
1. Read the TRD spoke + tasks, scan the real code paths the work touches, and **summarize the implementation scope** for the user to confirm. **For UI platforms, also capture the Design references** — the Figma frame link or committed image (`docs/development/<feature-name>/design/<screen>.png`) each screen must match, plus any specific needs (states, breakpoints, motion) — into the plan's *Design references* section; ask the user for them if missing. **Present the summary, then STOP and wait for confirmation** before the architecture layout (don't plan on a stale or unconfirmed understanding).
