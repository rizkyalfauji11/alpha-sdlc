...
**Verify-only:** `do-testing` catches and reports — it does **not** fix. A failed parity / integration / E2E check is a finding → back to `do-development`, like any failing test.
...
3. **Coverage report.** Confirm every AC maps to at least one passing test at the right level; show the **pyramid coverage** (which ACs are covered at API / UI / Integration / E2E). **Flag any uncovered AC, failing test, or level marked manual** (e.g. env unavailable) explicitly — don't paper over gaps. Any failed parity / integration / E2E check is a finding to hand **back to `do-development`**, not something to fix here.
