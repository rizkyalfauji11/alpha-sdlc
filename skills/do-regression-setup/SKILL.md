...
description: Step 0 of the regression track — set up the QA project for testing the built app. You give what you have (documents, a web link, the app file, test accounts — anything can be skipped and added later); the AI opens the app, walks through every screen like a new QA member learning the product, and writes the QA handbook (docs/qa-basics/) — what the app is, the test-user list (one per situation), and the test checklist with everything not yet covered marked loudly. Needs no access to the app's source code, ever. Use to set up the regression project, bootstrap the QA/testing profile, onboard a test repo, or before the first regression planning. Triggers on "regression setup", "set up the regression project", "QA project setup", "testing project setup", "/do-regression-setup".
---
...
## Tests the app like a real user — no source code, ever
...
## The boundary: like a real user, plus two helpers
...
## The test-user list — one user per situation, derived so nothing is missed
