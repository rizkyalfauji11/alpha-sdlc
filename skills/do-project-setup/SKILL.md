...
| `domain-model.md` | **Logical entities + relationships + cross-feature ownership** (owner = source of truth; consumers read via owner) — the shared truth interdependent features bind to; flags cross-feature contradictions | cache; cross-links `database`/`api-reference`/`feature-map` |
