...
**Continue without you.** Every document section, plan stage, and test result is a hard gate: plain-language summary first, engineering detail below, then it stops. No "generate the whole document", no batched approvals, no default it proceeds on if you go quiet. You review one small change at a time instead of one enormous diff at the end.
