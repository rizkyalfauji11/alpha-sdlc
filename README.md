...
Only four things halt the chain, because nothing can be decided: **verification tooling that fails** (a browser/emulator that won't boot is reported with its fix, never skipped), **an input that doesn't exist** (a design, test account, or seed access never provided), **external writes** (git push, Jira — those always ask), and **a fix that has failed three times** (the design is wrong, not the patch — it stops and asks). Every verifier runs at full strength either way — what you trade is review-per-diff, not checks.
