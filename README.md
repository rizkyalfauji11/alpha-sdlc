...
> **Regression track (in progress)** — `/do-regression-setup` + `/do-regression-authoring`: a stand-alone QA set that black-box tests the **built** app (deployed URL / installed APK-IPA) with no repo access, writing its own `docs/qa-basics/` profile and coverage map. Suite runs & triage at scale are next. Deployment + monitoring are roadmap.
